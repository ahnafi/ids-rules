...
ips =
{
    ...
    rules = [[
	include /usr/local/etc/snort/rules/DVWA/csrf.rules
	include /usr/local/etc/snort/rules/DVWA/sqlinjection.rules
	include /usr/local/etc/snort/rules/DVWA/xss.rules
    ]]

	-- include /usr/local/etc/rules/pulledpork.rules
	-- include /usr/local/etc/snort/rules/comunity.rules

	-- include /usr/local/etc/snort/rules/ping.rules
...
alert_json =
{
    file = true,
    limit = 1000,
    fields = 'timestamp msg pkt_num proto pkt_gen pkt_len dir src_addr src_port dst_addr dst_port'
}
